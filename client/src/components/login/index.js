...
import { BackHandler, Animated, ToastAndroid, Platform, View, Text, Image, ImageBackground, Dimensions, TextInput, TouchableOpacity, AsyncStorage, ActivityIndicator, Alert, Keyboard } from 'react-native'
...
class Login extends Component {
  ...
  _sendDataForLogin () {
    ...
    let callback = (input) => {
      const self = this
      input.status === 200 ? (
      AsyncStorage.setItem('headers', JSON.stringify(input.headers)),
      AsyncStorage.setItem('dataUser', JSON.stringify(input.data.result)),
      ...
      Alert.alert(
        input.data.result.title,
        input.data.result.message,
